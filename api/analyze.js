...
반드시 아래 JSON 형식으로만 응답하세요 (마크다운 코드블록 없이 순수 JSON):
{
  ...
  try {
    ...
    return { summary: text, prediction: '', sector: '', time_horizon: '중기(3년)', key_signals: [], risk_factors: [], countries_leading: [] }
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    const { data: trends } = await supabase.from('trends').select('id, keyword')
    let analyzed = 0
    ...
    for (const trend of trends) {
      ...
      const score = calcTrendScore(evidenceList)
      ...
      const report = await generateReport(trend.keyword, evidenceList, score)
    ...
    res.status(200).json({ ok: true, message: `${analyzed}개 트렌드 분석 완료` })
  } catch (e) {
    console.error(e)
